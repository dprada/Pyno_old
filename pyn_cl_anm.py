...
class anm_classic():
    ...
    def build_correl(self,modes='ALL'):
        ...
        self.correl=f_gnm.correlation(self.eigenvects2,self.eigenvals,list_modes,num_nodes,num_modes)
    ...
    def write(self):
        ...
        for aa in range(3*len(self.contact_map)):
            ...
            num_modes=1
            list_modes=[]
            list_modes.append(aa+1)
            self.correl=f_gnm.correlation(self.eigenvects2,self.eigenvals,list_modes,num_nodes,num_modes)
            for ii in range(len(self.contact_map)):
                    f_vects.write("%s %f\n" %(self.system.atom[ii].pdb_index,self.correl[ii,ii])
            f_vects.write(" \n")
